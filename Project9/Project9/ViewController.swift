...
class ViewController: UITableViewController {
    ...
    private func fetchPetitions() {
        let activeTabIndex = navigationController?.tabBarItem.tag
        performSelector(inBackground: #selector(fetchJSON(for:)), with: activeTabIndex)
    }
    
    @objc private func fetchJSON(for activeTabIndex: Int) {
        ...
        performSelector(onMainThread: #selector(showLoadingError), with: nil, waitUntilDone: false)
    }
    
    @objc private func showLoadingError() {
    ...
    private func parse(json: Data) {
        ...
        if let jsonPetition = try? decoder.decode(Petitions.self, from: json) {
            ...
            // Using the next line causes the following warning to show: "UITableViewController.tableView must be used from main thread only"
